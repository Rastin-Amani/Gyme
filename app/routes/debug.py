...
def debug_plan_expand(request: Request):
    pb = request.state.pb
...
def debug_plan_items(request: Request):
    pb = request.state.pb
    id = "guhobjjzu2jt2kv"
    tenant = request.state.tenant.id
    ...
    return {"plan": plan_data, "items": item_data}


@router.get("/traineeitems")
def debug_trainee_items(request: Request):
    pb = request.state.pb
    tenant = request.state.tenant.id
...
def debug_user_items(request: Request):
...
@router.get("/usertest")
def debug_user_test(request: Request):
    pb = request.state.pb
...
def debug_user_plans(request: Request):
    pb = request.state.pb
    tenant = request.state.tenant.id
    user = request.state.user
    ...
    return {"tenant": tenant_name, "user": user, "plans": plans}
